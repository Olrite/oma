...
public class ItemHighlight extends Module {
    ...
    private static boolean isIllegalFish(ItemStack stack) {
        ...
        TropicalFishEntity.Pattern pattern = stack.get(DataComponentTypes.TROPICAL_FISH_PATTERN);
        if (base != null || patternColor != null || pattern != null) {
            TropicalFishEntity.Variant variant = new TropicalFishEntity.Variant(
                pattern != null ? pattern : TropicalFishEntity.DEFAULT_VARIANT.pattern(),
                base != null ? base : TropicalFishEntity.DEFAULT_VARIANT.baseColor(),
                patternColor != null ? patternColor : TropicalFishEntity.DEFAULT_VARIANT.patternColor()
            );
        ...
        int packed = tag.getInt("BucketVariantTag", -1);
        if (packed < 0) return false;

        return isBlackIllegalVariant(new TropicalFishEntity.Variant(packed));
    }

    private static boolean isBlackIllegalVariant(TropicalFishEntity.Variant variant) {
