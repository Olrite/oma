...
public class ElytraPlus extends Module {
    ...
    // Generic inventory equip state machine (used for double-tap launch & land-equip-chestplate)
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        // Auto Liftoff: keybind-triggered equip -> jump -> open wings at apex -> firework.
        ...
        // Double-tap jump while airborne without an elytra: equip one and launch.
        ...
        // Redeploy flow: jump then activate fall-flying once airborne.
            ...
            switch (onLanding.get()) {
                case RedeployElytra -> {
                    if (!hasElytra) {
                        ...
                        if (requireForwardMovement.get() && !mc.options.forwardKey.isPressed()) {
                            wasFlying = false;
                        } else {
                            tickCounter++;
    ...
    /**
     * Generic staged equip: move the target item into the hotbar if needed, right-click to
     * equip it, then move whatever was previously in that hotbar slot back to where it came from.
     */
        ...
        switch (equipStage) {
            case 1 -> {
                if (equipWorkingSlot >= 9) {
                    ...
                    equipHotbarOriginalItem = mc.player.getInventory().getStack(hotbarSlot).copy();
    ...
    /**
     * Kicks off the Auto Liftoff sequence: equip an elytra (if not already worn), then jump.
     */
    ...
    /**
     * Moves a firework rocket into the hotbar if one isn't already there, so it can be used once flying.
     */
    private void ensureFireworkInHotbar() {
        for (int i = 0; i < 9; i++) {
            if (mc.player.getInventory().getStack(i).getItem() == Items.FIREWORK_ROCKET) return;
        }
    ...
    /**
     * Drives the jump -> apex -> open-wings part of Auto Liftoff once the elytra/firework are equipped.
     */
    private void tickLiftoff(boolean isOnGround) {
        ...
        switch (liftoffStage) {
            case Jump -> {
                if (isOnGround) {
                    mc.player.jump();
                    lastLiftoffMotionY = mc.player.getVelocity().y;
