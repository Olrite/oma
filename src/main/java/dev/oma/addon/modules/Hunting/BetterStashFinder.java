...
public class BetterStashFinder extends Module
{
    ...
    {
        super(Main.HUNT, "Better Stash Finder", "Jefff mod's BetterStashFinder but with more features.");
    }
    ...
    public WWidget getWidget(GuiTheme theme) {
        ...
        WButton addToWaypoints = list.add(theme.button("Add All to Waypoints")).widget();
        ...
        addToWaypoints.action = () -> {
            WaypointSet waypointSet = getWaypointSet();
            if (waypointSet == null) {
                error("Unable to get waypoint set.");
                return;
            }
            int added = 0;
            for (Chunk chunk : chunks) {
                if (getWaypointByCoordinate(chunk.x, chunk.z) == null) {
                    addToWaypoints(waypointSet, chunk);
                    added++;
                }
            }
            SupportMods.xaeroMinimap.requestWaypointsRefresh();
            info("Added (highlight)%d(default) stashes to waypoints.", added);
        };
    ...
    private File getJsonFile() {
        return new File(new File(new File(MeteorClient.FOLDER, "stash-finder-plus"), Utils.getFileWorldName()), "stashes.json");
    }

    private File getCsvFile() {
        return new File(new File(new File(MeteorClient.FOLDER, "stash-finder-plus"), Utils.getFileWorldName()), "stashes.csv");
    }
