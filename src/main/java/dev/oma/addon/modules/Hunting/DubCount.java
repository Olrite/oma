...
public class DubCount extends Module {
    ...
    public DubCount() {
        super(Main.HUNT, "Dub Counter", "Counts how many double chests are in render distance.");
    }
    ...
    private void applyCount(int length, String sourceLabel) {
        ...
        if (showNotifications.get() || !autoUpdate.get() || isFirstCount) {
            ...
            LogUtils.info(Formatting.GRAY + "There are roughly " + Formatting.WHITE + formatCount(value) + Formatting.GRAY + " " + unit + " (" + length + " chest blocks) " + sourceLabel + ".");
        }
        isFirstCount = false;
    }
