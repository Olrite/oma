...
public class PortalPredicter extends Module {
        ...
        for (int x = center.getX() - horizontalRadius; x <= center.getX() + horizontalRadius; x++) {
            for (int z = center.getZ() - horizontalRadius; z <= center.getZ() + horizontalRadius; z++) {
                ...
                for (int y = yMax; y >= yMin; y--) {
                    ...
                    if (isValidPortalSpawn(level, candidate)) {
    ...
    private boolean isValidPortalSpawn(Level level, BlockPos basePos) {
        List<BlockPos> requiredSolidBlocks = List.of(
            basePos,
            basePos.relative(Direction.NORTH, 1),
            basePos.relative(Direction.NORTH, 2),
            basePos.relative(Direction.NORTH, 3)
        );
    ...
    private void onRender(Render3DEvent event) {
        for (BlockPos portalBase : portalBases) {
            for (int i = 0; i < 5; i++) {
                event.renderer.box(portalBase.relative(Direction.NORTH, i), sideColor.get(), lineColor.get(), shapeMode.get(), 0);
            }
        }
    }
}
