...
public class AutoRekit extends Module {
    ...
    private AbstractContainerMenu pendingHandler = null;
    ...
    private void onOpenScreen(OpenScreenEvent event) {
        ...
        if (event.screen instanceof AbstractContainerScreen<?> && !(event.screen instanceof InventoryScreen)) {
        ...
        if (mode.get() == RekitMode.OnKey) {
            ...
            if (pressed && !keyWasPressed) {
                ...
                if (mc.screen instanceof AbstractContainerScreen<?> && !(mc.screen instanceof InventoryScreen)) {
                    AbstractContainerMenu current = mc.player.containerMenu;
                    if (current != null && current != mc.player.inventoryMenu) handler = current;
                }
                queueKit(handler);
            } else if (!pressed) {
                keyWasPressed = false;
    ...
    private void queueKit(AbstractContainerMenu containerHandler) {
        if (mc.player == null) return;
        String name = loadout.get();
    ...
    private void executePendingOps() {
        if (pendingOps.isEmpty() || mc.gameMode == null) return;

        if (getSyncId(mc.player.containerMenu) != pendingSyncId) {
            pendingOps.clear();
            return;
        }
        ...
        while (budget-- > 0) {
            ...
            if (op.to() == -1) {
                mc.gameMode.handleContainerInput(pendingSyncId, op.from(), 0, ContainerInput.QUICK_MOVE, mc.player);
            } else {
                mc.gameMode.handleContainerInput(pendingSyncId, op.from(), 0, ContainerInput.PICKUP, mc.player);
                mc.gameMode.handleContainerInput(pendingSyncId, op.to(), 0, ContainerInput.PICKUP, mc.player);
                if (op.returnTo() != -1) {
                    mc.gameMode.handleContainerInput(pendingSyncId, op.returnTo(), 0, ContainerInput.PICKUP, mc.player);
                }
            }
        }
    }
    ...
    private List<MoveOp> buildPlan(HashMap<Integer, Item> kit, AbstractContainerMenu containerHandler) {
        List<MoveOp> plan = new ArrayList<>();
        if (mc.player == null) return plan;

        Set<Integer> kitSlots = kit.keySet();
        boolean hasContainer = containerHandler != null && containerHandler != mc.player.inventoryMenu;
        ...
        for (int i = 0; i < containerSize; i++) {
            ItemStack s = containerHandler.getSlot(i).getItem();
            conItem[i] = s.getItem();
    ...
    private int toScreenSlot(int invSlot, AbstractContainerMenu containerHandler) {
        if (containerHandler != null && containerHandler != mc.player.inventoryMenu) {
    ...
    private int getSyncId(AbstractContainerMenu containerHandler) {
        return containerHandler != null && containerHandler != mc.player.inventoryMenu
    ...
    public void saveKit(String kitName) {
        ...
        for (int i = 0; i < 36; i++) {
            ...
            if (!stack.isEmpty()) {
                ...
                if (id != null) kitData.put(String.valueOf(i), id.toString());
            }
        }
